...
Tauri Agrícola facilitates spot transactions by combining active market offers, integrated logistics coordination, and a secure operational structure, covering the process from negotiation to settlement

### Capital Deployment

Capital allocated to this grain strategy:

Is used solely for physical grain trading
...
Altura provides stablecoin liquidity to Travessia, which is allocated to Tauri, a local commodity trade finance operator that uses the capital to fund short-duration grain transactions. 

Tauri pays producers immediately when grain is loaded, coordinates logistics and sale execution, hedges commodity and FX exposure where needed, and turns the capital over through repeatable spot-market transactions in corn, soy, and sorghum. 

The yield comes from financing this working-capital gap: Tauri can access faster, more flexible capital than traditional bank credit, while vault depositors receive exposure to real-world trade finance yields backed by essential commodity flows.

Travessia Credit’s Tauri Vault is designed to finance high-velocity Brazilian grain flows through a structured, on-chain credit vault. 

Depositors provide stablecoin liquidity into the vault, which is allocated to Tauri, a local commodity trade finance operator that uses the capital to fund short-duration grain transactions
